(() => {
  ...
  function renderCloud(){const target=$('#cloudView');if(!target)return;if(!state.user){target.innerHTML=emptyState('Sign in to use Cloud Gaming','Cloud Gaming is available to verified Flash Games accounts.','cloud');refreshIcons(target);return;}if(!isVerifiedUser()){target.innerHTML=`<div class="page-head"><div><span class="eyebrow">CLOUD GAMING</span><h1>Verification required</h1><p>Cloud Gaming is reserved for verified Flash Games accounts.</p></div></div><div class="empty glass">${icon('badge-check')}<h3>Get verified</h3><p>Your account is signed in, but verification has not been granted yet.</p></div>`;refreshIcons(target);return;}const cloudGames=Array.isArray(window.FlashCloudGames)?window.FlashCloudGames:[];const cards=cloudGames.length?'<div class="game-grid">'+cloudGames.map(game=>'<article class="game-card cloud-game-card"><img src="'+esc(game.cover||'./offline/logo.png')+'" alt="'+esc(game.name||'Cloud Game')+'"><div class="game-card-body"><h3>'+esc(game.name||'Cloud Game')+'</h3><p>'+esc(game.description||'Play in the cloud.')+'</p><button class="play-btn" data-cloud-url="'+esc(game.url||'')+'">Play in Cloud</button></div></article>').join('')+'</div>':'<div class="empty glass">'+icon('cloud-lightning')+'<h3>No cloud games yet</h3><p>Add games to window.FlashCloudGames in data.js.</p></div>';target.innerHTML='<div class="page-head"><div><span class="eyebrow">CLOUD GAMING</span><h1>Play without installing</h1><p>Your verified account can access cloud-ready games.</p></div></div>'+cards;target.querySelectorAll('[data-cloud-url]').forEach(button=>button.onclick=()=>{const url=button.dataset.cloudUrl;if(url)window.open(url,'_blank','noopener,noreferrer')});refreshIcons(target);}
  ...
  function setRoute(route) {
    const valid = ['home', 'games', 'library', 'store', 'social', 'cloud', 'updates', 'admin']; state.route = valid.includes(route) ? route : 'home';
    ...
    if (state.route === 'home') renderHome(); if (state.route === 'games') renderGames(); if (state.route === 'library') renderLibrary(); if (state.route === 'store') renderStore(); if (state.route === 'social') renderSocial(); if (state.route === 'cloud') renderCloud(); if (state.route === 'updates') renderUpdates(); if (state.route === 'admin') renderAdmin();
